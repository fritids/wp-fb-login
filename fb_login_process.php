...
//Prevent the user in accessing this file
...
if(! isset($_POST['redirectTo']) || ! $_POST['redirectTo'])
    die("Error: Missing POST Data (redirect)");
    
$redirectTo = $_POST['redirectTo'];
$pfb_log .= "WP: Found redirect URL ($redirectTo)\n";

//Include Facebook, making sure another plugin didn't already do so
if(class_exists('Facebook')) {
    $pfb_log .= "WP: WARNING - Another plugin has already included the Facebook API. "
    ...
    require_once('facebook.lib/php-sdk-3.1.1/facebook.php');
}

$pfb_log .= "FB: Initiating Facebook connection...\n";

//Connect to FB and make sure we've got a valid session (we should already from the cookie set by JS)  
$pfb_log .= "FB: Initiating Facebook connection...\n";
$facebook = new Facebook(array('appId' => get_option($pfb_opt_app_id),
    'secret' => get_option($pfb_opt_api_sec), 'cookie' => true));
    
try { 
    $fb_uid = $facebook->getUser(); 
} catch (FacebookApiException $e)  { 
    die("Error: Exception when getting the Facebook userid. Please verify your API " .
        "Key and Secret."); 
}

if (! $fb_uid) {
    die("Error: Failed to get the Facebook user session. Please see FAQ37 on the plugin" . 
        " documentation page. UID: $fb_uid"); 
}

$pfb_log .= "FB: Connected to session (uid $fb_uid)\n";

//Get the user info from FB
try {
    $fbuser = $facebook->api('/me');
    /* $fbuser['current_location'] = $facebook->api(array('method' => 'users.getinfo',
        'uids' => $fbuser['id'], 'fields' => 'current_location', 'callback' => '')); */
    ...
    die("Error: Could not access the Facebook API client (failed on users_getInfo($fb_uid))." . 
        "  Result: " . print_r($fbuserarray, true) . "; " . $e );
}

if(! $fbuser ) {
    die("Error: Could not access the Facebook API client (failed on users_getInfo($fb_uid))." .
        "  Result: " . print_r($fbuserarray, true) ); 
}

$pfb_log .= "FB: Got user info (".$fbuser['name'].")\n";
...
if( strlen($fbuser['email']) != 0 && strpos($fbuser['email'],
    'proxymail.facebook.com') === FALSE ) {
    $pfb_log .= "FB: Email privilege granted (" .$fbuser['email'] . ")\n";
    $userRevealedEmail = true;
} else if( strlen($fbuser['email']) != 0 ) {
    $pfb_log .= "FB: Email privilege granted, but only for an anonymous proxy address (" .
        $fbuser['email'] . ")\n";
} else {
    $pfb_log .= "FB: Email priviledge denied.\n";
    $fbuser['email'] = "FB_" . $fb_uid . $pfb_default_email;
}
...
if(!isset($wp_users)) {
    ...
    $sql = "SELECT user_id, user_id AS ID, user_login, display_name, user_email, meta_value ".
           ...
           "AND {$wpdb->users}.ID IN (SELECT user_id FROM {$wpdb->usermeta} " .
            "WHERE meta_key = '$pfb_uid_meta_name' AND meta_value = '$fb_uid')";
...
$pfb_log .= "WP: Searching " . count($wp_users) . " existing candidates by meta...\n";
foreach ($wp_users as $wp_user)
{
    $meta_uid  = get_user_meta($wp_user->ID, $pfb_uid_meta_name, true);
    if($meta_uid && $meta_uid == $fb_uid ) {
        $user_data       = get_userdata($wp_user->ID);
        $user_login_id   = $wp_user->ID;
        $user_login_name = $user_data->user_login;
        $pfb_log .= "WP: Found existing user by meta (" . $user_login_name . ")\n";
        break;
    }
}
...
if (! $user_login_id && $userRevealedEmail ) {
    $pfb_log .= "WP: Searching for user by email address...\n";
    if ($wp_user = get_user_by('email', $fbuser['email'])) {
        $user_login_id = $wp_user->ID;
        $user_data = get_userdata($wp_user->ID);
        $user_login_name = $user_data->user_login;
        $pfb_log .= "WP: Found existing user (" . $user_login_name .
            ") by email (" . $fbuser['email'] . ")\n";
    }
}
...
if($user_login_id)
{
    ...
    $updateEmail = false;
    if( strpos($user_data->user_email, $jfb_default_email) !== FALSE &&
        strpos($fbuser['email'], $jfb_default_email) === FALSE ) {
        $pfb_log .= "WP: Previously DENIED email has now been allowed; updating to (" .
            $fbuser['email'] . ")\n";
        $updateEmail = true;
    }
    
    //Check 2: It was previously allowed, but only as an anonymous proxy.  They've now revealed their "true" email.
    if( strpos($user_data->user_email, "@proxymail.facebook.com") !== FALSE &&
            strpos($fbuser['email'], "@proxymail.facebook.com") === FALSE ) {
        $pfb_log .= "WP: Previously PROXIED email has now been allowed; updating to (" . 
            $fbuser['email'] . ")\n";
        $updateEmail = true;
    }
...
if(! $user_login_id) {
    $pfb_log .= "WP: No user found. Automatically registering (FB_". $fb_uid . ")\n";
    $user_data = array();
    ...
    $user_data = apply_filters('wpfb_login_insert_user', $user_data, $fbuser );
    $user_data = apply_filters('wpfb_login_inserting_user', $user_data, array(
        'WP_ID' => $user_login_id, 'FB_ID' => $fb_uid, 'facebook' => $facebook,
            'FB_UserData' => $fbuser) );
    ...
    if( is_wp_error($user_login_id) ) {
        die("Error: wp_insert_user failed!<br/><br/>".
            "If you get this error while running a Wordpress MultiSite installation, it means " . 
                "you'll need to purchase the " . 
                    "<a href=\"$jfb_homepage#premium\">premium version</a> of this plugin to " . 
                        "enable full MultiSite support.<br/><br/>".
            "If you're <u><i>not</i></u> using MultiSite, please report this bug to the " . 
                "plugin author on the support page " . 
                    "<a href=\"$jfb_homepage#feedback\">here</a>.<br /><br />".
            "Error message: " . (function_exists(array(&$user_login_id, 'get_error_message')) ?
                $user_login_id->get_error_message() : "Undefined") . "<br />".
            "WP_ALLOW_MULTISITE: " . (defined('WP_ALLOW_MULTISITE') ?
                constant('WP_ALLOW_MULTISITE') : "Undefined") . "<br />".
...
update_user_meta($user_login_id, $pfb_uid_meta_name, $fb_uid);
$pfb_log .= "WP: Updated usermeta ($pfb_uid_meta_name)\n";
...
if($fbuser['pic_square'] ) {
    ...
    $pfb_log .= "WP: Updated avatars (" . $avatarThumb . ")\n";
} else {
    update_user_meta($user_login_id, 'facebook_avatar_thumb', '');
    update_user_meta($user_login_id, 'facebook_avatar_full', '');
    $pfb_log .= "FB: User does not have a profile picture; clearing cached avatar (if present).\n";
}

//Log them in
$rememberme = apply_filters('wpfb_login_rememberme',
    isset($_POST['rememberme'])&&$_POST['rememberme']);
...
$jfb_log .= "   WP User : $user_login_name (" . 
    admin_url("user-edit.php?user_id=$user_login_id") . ")\n";
